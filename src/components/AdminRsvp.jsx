...
export default function AdminRsvp() {
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `RSVP_Chelsea_Ranu_${new Date().toLocaleDateString("id-ID").replace(/\//g, "-")}.xlsx`,
    );
  };
  ...
  const totalJiwa = data
    ...
    .reduce((acc, r) => acc + (r.jumlah_tamu || 1), 0);
      ...
      <div className="min-h-screen bg-sage-dark flex items-center justify-center px-6">
        <div className="w-full max-w-sm space-y-8 text-center">
          <div className="space-y-2">
            <p className="font-cormorant text-gold tracking-[0.3em] text-xs uppercase">
              Admin
            </p>
            <h1 className="font-cormorant text-cream text-3xl italic">
              Chelsea & Ranu
            </h1>
            <p className="font-elle text-cream/40 text-xs tracking-widest">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            />
            {wrongPass && (
              <p className="font-elle text-red-400 text-xs tracking-wider">
                Password salah.
              </p>
            )}
            <button
  ...
  return (
    <div className="min-h-screen bg-cream font-elle">
      <div className="bg-sage-dark px-6 py-8 text-center">
        <p className="font-cormorant text-gold tracking-[0.3em] text-xs uppercase mb-1">
          Admin Panel
        </p>
        <h1 className="font-cormorant text-cream text-3xl italic">Data RSVP</h1>
        <p className="font-cormorant text-cream/50 text-base mt-1">
          Chelsea & Ranu · 19 April 2026
        </p>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div
              key={s.label}
              className="bg-white/60 border border-gold/15 rounded-sm p-4 text-center"
            >
              <p className={`font-cormorant text-3xl font-light ${s.color}`}>
                {s.value}
              </p>
              <p className="font-elle text-sage-dark/50 text-xs tracking-widest mt-1">
                {s.label}
              </p>
            </div>
          ))}
        ...
        <div className="bg-white/60 border border-gold/15 rounded-sm overflow-hidden">
          {loading ? (
            <div className="py-16 text-center">
              <p className="font-cormorant text-sage-dark/40 text-lg italic">
                Memuat data...
              </p>
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gold/15 bg-sage-dark/5">
                    <th className="text-left px-4 py-3 font-elle text-xs tracking-widest text-sage-dark/50 uppercase">
                      No
                    </th>
                    <th className="text-left px-4 py-3 font-elle text-xs tracking-widest text-sage-dark/50 uppercase">
                      Nama
                ...
                <tbody>
                  {filteredData.map((row, i) => (
                    ...
                    >
                      <td className="px-4 py-3 font-elle text-sage-dark/40 text-sm">
                        {i + 1}
                      </td>
                      <td className="px-4 py-3 font-cormorant text-sage-dark text-base">
                        {row.nama}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`font-elle text-xs tracking-wider px-2 py-1 rounded-sm
                          ${
                            row.status === "hadir"
