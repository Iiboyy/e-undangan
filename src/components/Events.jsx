...
export default function Events({ guest }) { // 🔥 Terima props guest dari induk
  const sectionRef = useRef(null)

  // Fungsi untuk menentukan jam resepsi berdasarkan kloter
  const getReceptionTime = () => {
    if (!guest || !guest.kloter) return '11.00 – 14.00 WIB' // Jam default jika tidak ada data

    // Logika pembagian jam per kloter
    switch (parseInt(guest.kloter)) {
      case 1: return '11.00 – 12.00 WIB'
      case 2: return '12.00 – 13.00 WIB'
      case 3: return '13.00 – 14.00 WIB'
      default: return '11.00 – 14.00 WIB'
    }
  }
            ...
            <div key={i} className="event-card opacity-0 group">
              <div className="h-full border border-gold/20 bg-white/40 backdrop-blur-sm rounded-sm px-8 py-10 text-center space-y-5 transition-all duration-500 group-hover:border-gold/50">
                ...
                <div className="space-y-1">
                  {/* Tampilkan jam yang sudah dinamis */}
                  <p className="font-cormorant text-sage-dark text-lg font-bold">{ev.time}</p> 
                ...
                <a href={ev.mapsUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-2 px-6 py-2.5 border border-gold/40 text-gold font-elle text-xs tracking-widest uppercase hover:bg-gold hover:text-cream transition-all duration-300">
                  Lihat Lokasi
                </a>
              </div>
            </div>
